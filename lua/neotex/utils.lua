...
local function join_paths(...) return table.concat(vim.tbl_flatten({...}), "/") end
...
function M.is_executable(program) return vim.fn.executable(program) == 1 end

function M.assert_executable(program, label)
    if M.is_executable(program) then return true end

    logger.error(string.format("%s is not executable: %s", label or "Program",
                               program))
    return false
end

function M.is_tex_file(path) return path:sub(-4) == ".tex" end
...
function M.resolve_tex_path(target)
    ...
    if not M.is_tex_file(candidate) then candidate = candidate .. ".tex" end
...
function M.ensure_dbus()
    if vim.env.DBUS_SESSION_BUS_ADDRESS and vim.env.DBUS_SESSION_BUS_ADDRESS ~=
